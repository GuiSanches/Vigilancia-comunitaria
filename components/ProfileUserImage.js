...
import gatao from "../assets/images/DefaultUserAlt.jpg";
...
export default ProfileImage = (props) => {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const userImage = await firebase.FIREBASE.storage()
          .ref(`ProfileImg/${firebase.token}.jpg`)
          .getDownloadURL();
        setImage(userImage);
      } catch (e) {
          console.log('nao deua')
      }
  ...
  const pickImage = async () => {
    try {
      ...
      firebase.FIREBASE.storage()
        .ref()
        .child(`ProfileImg/${firebase.token}.jpg`)
        .put(blob)
        .on(
          ...
          (snapshot) => {
            ...
            if (
              snapshot.state === firebase.FIREBASE.storage.TaskState.SUCCESS
            ) {
