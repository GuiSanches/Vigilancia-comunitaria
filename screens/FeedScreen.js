...
const getPostTime = (then) => {
  ...
  if (duration.years() > 0) {
    ...
    else return "há " + duration.hours() + " h " + duration.minutes() + " min";
...
const FeedScreen = (props) => {
  ...
  const RefreshPosts = async (_) => {
    try {
      const QuerySnaphot = await firebase.FIREBASE.firestore()
        .collection("ALERT")
        .where("deleted", "==", false)
        .limit(150)
        .get();
      ...
      const documents = await Promise.all(
        QuerySnaphot.docs // Array QueryDocumentSnapshot
          .map(async (document) => {
